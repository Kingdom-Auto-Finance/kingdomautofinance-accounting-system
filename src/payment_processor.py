...
def process_payments():
    ...
    try:
        ...
        return
    try: # Optional: Init drive service early
        gutils.get_drive_service() 
    except ConnectionError as e:
         logger.warning(f"Could not initialize Drive client on startup: {e}.")

    # --- 1. Read Payments Log and Perform Initial Validation ---
    ...
    validated_payment_list = [] # Stores dicts {'original_index': idx, 'data': {loanid_orig:.., loanid_upper:.., date:.., amount:..}}
            ...
            if not payment_date_header_original: df_log_sheet_state.loc[index, status_col_original_casing] = "Error - Missing PaymentDate Column"; has_critical_error = True
                ...
                if not raw_payment_date: df_log_sheet_state.loc[index, status_col_original_casing] = "Error - Missing PaymentDate"; has_critical_error = True
                    ...
                    try: parsed_date = pd.to_datetime(raw_payment_date, format='%Y-%m-%d', errors='raise')
                        ...
                        try: parsed_date = pd.to_datetime(raw_payment_date, errors='raise'); logger.warning(f"Log idx {index}: Date '{raw_payment_date}' not YYYY-MM-DD...")
        ...
        if not has_critical_error:
            if not payment_amount_header_original: df_log_sheet_state.loc[index, status_col_original_casing] = "Error - Missing PaymentAmount Column"; has_critical_error = True
                ...
                if not raw_payment_amount: df_log_sheet_state.loc[index, status_col_original_casing] = "Error - Missing PaymentAmount"; has_critical_error = True
                    ...
                    except (ValueError, TypeError) as e: df_log_sheet_state.loc[index, status_col_original_casing] = "Error - Invalid PaymentAmount"; has_critical_error = True; parsed_amount = pd.NA
    ...
    validated_payment_list.sort(key=lambda p: (p['data'][LOAN_ID_COL_LOWER], p['data'][PAYMENT_DATE_COL_LOWER])) # Group by LoanID, then sort by date
    
    from collections import defaultdict
    grouped_payments = defaultdict(list)
    for payment_item in validated_payment_list:
        # Use internal uppercase loan_id for grouping key
    ...
    processed_amortization_sheets = {} # Cache {loan_id_internal: {'df': schedule_df, 'original_headers': headers, 'sheet_id': id}}

    # --- 3. Process Payments Loan by Loan ---
    for loan_id_internal, payment_items_for_loan in grouped_payments.items():
        
        loan_id_original_case = payment_items_for_loan[0]['data']['loanid_original_case'] # Get original casing from first item
        ...
        if not amortization_sheet_id:
            logger.error(f"Amort. Sheet ID for LoanID '{loan_id_original_case}' not found in Drive. Marking all payments for this loan as error.")
        ...
        try:
            ...
            # Prepare terms and schedule DataFrames for processing
            ...
            for payment_item in payment_items_for_loan:
                ...
                logger.debug(f"Applying payment from log index {original_log_index} (Date: {actual_payment_date_str}, Amt: {actual_payment_amount_from_log:.2f}) to {loan_id_internal}'s schedule.")
                ...
                try: # Inner try-except for individual payment calculation/application
                    ...
                    for i, sr_iter in schedule_df.iterrows():
                        ...
                        # Find first row not fully paid or still marked due/partial
                        if pd.isna(apds) or cs in ["due", "partially paid", ""]: 
                            target_row_idx = i
                            break
                    
                    if target_row_idx == -1:
                        raise ValueError("No Due Payment Slot found in schedule")

                    # Extract/Validate data from schedule target row
                    ...
                    if pd.isna(due_date_dt_sched): raise ValueError("Invalid DueDate in Sched")
                    if pd.isna(beginning_balance_for_calc): raise ValueError("Invalid BeginBal in Sched")
                    ...
                    # Update the schedule DataFrame IN MEMORY for this payment
                    ...
                    # If this specific payment application was successful, mark log entry as 'Processed' for now
                    # We only finalize the write-back if the whole loan's processing succeeds
                    ...
                    logger.debug(f"Applied payment log index {original_log_index} successfully to in-memory schedule for {loan_id_internal}.")

                except Exception as payment_error: # Catch error applying a single payment
                    ...
                    df_log_sheet_state.loc[original_log_index, status_col_original_casing] = f"Error - {type(payment_error).__name__}"
                    df_log_sheet_state.loc[original_log_index, timestamp_col_original_casing] = datetime.now()
                    # If one payment fails, should we stop processing subsequent payments for this loan in this run?
                    # Decision: Yes, stop processing this loan if one payment fails to apply correctly.
                    loan_processing_failed = True 
                    break # Exit the inner loop over payments for this loan

            # --- After processing all payments for this loan (or breaking on error) ---
            if not loan_processing_failed:
                # --- Write Amortization Schedule Back ONCE ---
                logger.info(f"Attempting to save updated amortization schedule for {loan_id_internal}...")
                ...
                if gutils.update_worksheet_from_df(gs_client, amortization_sheet_id, "Schedule", schedule_df_to_write):
                    ...
                    # Statuses in df_log_sheet_state are already marked 'Processed' for successful payments
                else:
                    logger.error(f"Failed to save updated amortization schedule for {loan_id_internal}. Marking associated payments as error.")
                    # Revert status for payments processed in this batch for this loan
                    for item in payment_items_for_loan:
                        # Only revert if it was marked 'Processed' in this run
                        if str(df_log_sheet_state.loc[item['original_index'], status_col_original_casing]).lower() == 'processed':
                            ...
                            # Keep timestamp from when it was processed initially or update? Let's update.
                            df_log_sheet_state.loc[item['original_index'], timestamp_col_original_casing] = datetime.now() 
            else:
                # If loan_processing_failed was True, errors already marked on specific payments.
                 logger.warning(f"Skipping final amortization save for {loan_id_internal} due to error during payment application.")


        except Exception as loan_error: # Catch errors reading sheet/terms for the whole loan
            logger.error(f"Error processing LoanID {loan_id_internal} (Sheet read/init failed): {loan_error}", exc_info=True)
